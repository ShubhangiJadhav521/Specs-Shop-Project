...
app.post("/addCard",async(req,res) => {  // contact is action method POST
    try {
         res.send(req.body);
        
        const cardData = new card(req.body);  // calling schema
        await cardData.save();  // saving data
        res.status(201).send("index"); // submit ke bad jaha jana he wo page render kro
    } catch (error) {
